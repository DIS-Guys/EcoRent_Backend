...
- Use two runtime environments for now: `development` and `production`
- In `development`, keep `MOCK_S3=true`
- In `production`, set `MOCK_S3=false` and provide real AWS credentials
- Use `MONGO_URI` as the only database connection variable
...
4. Run ```npm run dev``` to launch back-end part
5. Now back-end is running ;)
6. Use ```npm test``` to run unit tests
7. Use ```npm run test:api``` to run integration tests with newman
